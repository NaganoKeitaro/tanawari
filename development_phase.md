...
| **HIERARCHY_DEFAULT_WIDTH** | `300` | 階層プレースメントのデフォルト幅（mm = 1尺） | `ShelfBlockEditor.tsx` |
| **VITE_IS_VERCEL** | `true/false` | Vercel環境判定フラグ（ビルド時に自動設定） | `vite.config.ts` |
...
| パラメータ名 | 設定値 | 説明 |
| :--- | :--- | :--- |
| **検索方式** | 全階層横断曖昧検索 | 8レベル全てのコード・名称を検索対象 |
| **複数ワード** | スペース区切りAND検索 | 全ワードが含まれるエントリのみ一致 |
| **ソート方式** | 関連度順（score降順） | 一致フィールド数に基づくスコアリング |
