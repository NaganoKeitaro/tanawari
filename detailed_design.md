...
### 1.1 共通前提（座標管理とシステム）
*   **店舗のフロア配置（2Dグリッド平面図）**: `StoreFixturePlacement` によりフロア左上を原点とする `positionX`, `positionY` と `direction` (角度) の絶対座標で管理される。
*   **棚割空間の配置（立面図/平面図）**: 左下や左端を原点とし、商品は `positionX`（左端からの相対距離）と `shelfIndex`（段数）で管理される。
*   フロア配置と棚割空間配置は `什器 (Fixture)` を軸として結合され、画面上で適切に紐づけて表示される。
*   **入力**: 標準棚割（商品リスト、ブロック配置）、店舗棚サイズ（幅）、商品売上ランク
...
3.  **商品削除ループ**:
    ...
    *   ※注意：JANコードを持たない特殊な商品群もルールに基づき許容される仕様へと柔軟に対応。
...
*   **テストデータや自動計算での適用**: プログラム的に同一商品が連続する場合に正規化を行い、冗長なレコード配列を避ける。
