...
import { swapBlock, expandBlockHierarchyPlacements } from './standardPlanogramRearrange';
...
import type { Fixture, StoreFixturePlacement, StorePlanogramHierarchyPlacement } from '../../data/types';
import { HierarchyPlacementEditor } from '../../components/planogram/HierarchyPlacementEditor';
...
function SinglePlanogramView({
    ...
    // 棚ブロック入れ替え（個店の階層プレースメント・商品をブロック単位でスワップ）
    // 標準棚割は変更せず、個店棚割内のデータのみ更新
    const handleSwapBlock = async (blockId: string, direction: SwapDirection) => {
        if (!planogram || !standardPlanogram) return;

        const blockMasters = blocks.map(b => ({ id: b.id, width: b.width, shelfCount: b.shelfCount }));

        // 個店用ブロック配置があればそちらを使用、なければ標準棚割のblocks
        const sourceBlocks = planogram.blocks ?? standardPlanogram.blocks;
        ...
        if (!packedBlocks) return; // 移動不可

        // 入れ替え後のブロック位置に基づいて、階層プレースメントを再生成
        const newHierarchyPlacements: typeof planogram.hierarchyPlacements = [];
        for (const pb of packedBlocks) {
            ...
            newHierarchyPlacements.push(...expandedH.map(eh => ({
                ...eh,
        ...
        // 商品も再配置（ブロック内商品がある場合）
        const newProducts: typeof current.products = [];
        for (const pb of packedBlocks) {
            const m = blocks.find(b => b.id === pb.blockId);
            if (!m) continue;
            for (const pp of m.productPlacements) {
                const product = products.find(p => p.id === pp.productId);
                if (!product) continue;
                newProducts.push({
                    id: crypto.randomUUID(),
                    productId: pp.productId,
                    shelfIndex: pb.positionY + pp.shelfIndex,
                    positionX: pb.positionX + pp.positionX,
        ...
        const updated = {
            ...current,
            blocks: packedBlocks, // 個店用ブロック配置を保存
            hierarchyPlacements: newHierarchyPlacements,
            products: newProducts.length > 0 ? newProducts : current.products,
    ...
    // 階層プレースメント更新
    const handleHierarchyUpdate = async (updatedHierarchyPlacements: StorePlanogramHierarchyPlacement[]) => {
        if (!planogram) return;
    ...
    return (
        <div className="card mb-lg animate-fadeIn">
            ...
            >
                ...
                {standardPlanogram && (() => {
                    // 個店用ブロック配置があればそちらを優先（入れ替え済み）、なければ標準棚割のblocks
                    ...
                    return <PlanogramExcelHeader blocks={blockInfos} totalWidthMm={planogram.width} selectedBlockId={selectedBlockId} onSelectBlock={handleSelectBlock} onSwapBlock={handleSwapBlock} />;
                })()}
                ...
                >
                    {/* 背景ブロック境界線 */}
                    {standardPlanogram && (planogram.blocks ?? standardPlanogram.blocks).map(block => {
                    ...
                    {(() => {
                        ...
                        const findBlockForProduct = (sp: StorePlanogramProduct): string | null => {
                            ...
                            for (const pb of (planogram.blocks ?? standardPlanogram.blocks)) {
                        ...
                        return (
                            ...
                            >
                                {/* 配置済み階層アイテム（編集可能） */}
                                <HierarchyPlacementEditor
                                    placements={planogram.hierarchyPlacements || []}
                                    shelfIndex={shelfIndex}
                                    planogramWidth={planogram.width}
                                    scale={SCALE}
                                    ...
                                    for (const groupKey of Object.keys(FIXTURE_GROUPS) as FixtureGroup[]) {
                                        ...
                                        for (const fType of groupTypes) {
                                            ...
                                            const stdPlan = storePlan
                                                ...
                                                : null;
                                            // 個店用ブロック配置があればそちらを優先
                                            const displayBlocks = storePlan?.blocks ?? stdPlan?.blocks;
                                            if (!displayBlocks || !displayBlocks.length) continue;
                                            ...
                                            // 各ブロックをどの什器に重なるか計算
                                            displayBlocks.forEach((pb, blockIdx) => {
                                                const master = blocks.find(b => b.id === pb.blockId);
                                                if (!master) return;
