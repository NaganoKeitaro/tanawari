...
import type { HierarchyLevel } from './productHierarchy';
...
export interface HierarchyPlacement {
  ...
  shelfIndex: number;
  positionX: number;    // mm
  width: number;        // mm（1フェイス分の幅、デフォルト300mm）
  faceCount: number;    // フェイス数（デフォルト1）
}
...
export interface StandardPlanogramHierarchyPlacement extends HierarchyPlacement {
  placedBlockId?: string;  // 配置元の StandardPlanogramBlock.id
}
...
export interface StorePlanogramHierarchyPlacement extends HierarchyPlacement {
