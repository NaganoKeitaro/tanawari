...
| 画面名 | 入出力項目例 | 機能・ボタン |
| :--- | :--- | :--- |
| **商品マスタ一覧/編集** | JAN、商品名、サイズ(W/H/D)、売上ランク、画像、カテゴリ（高度な階層フィルター） | 検索（階層別）、新規登録、編集、論理削除、CSVインポート（JANなし許容） |
| **店舗マスタ一覧/編集** | 店舗コード、名称、FMT、地域 | 検索、新規登録、編集、削除 |
| **什器マスタ一覧/編集** | 什器名、サイズ、段数、什器タイプ | 新規登録、編集（什器タイプ連携による自動補完あり） |
| **店舗什器配置** | 店舗選択、什器選択、配置座標(X/Y)、回転角 | グリッド上での2Dドラッグ＆ドロップ配置、回転、保存 |
...
*   **個店棚割エディタ**:
    ...
    *   **機能**:
        ...
        *   **手動調整**: 商品の追加・削除・フェイス変更・移動（同種商品のフェイス自動連結対応）。
        *   **2D平面図表示連携**: 実際の店舗内レイアウト位置の確認。
        *   **バリデーション**: 棚幅超過時や什器衝突時の警告表示。

#### 1.2.5 その他・分析機能 (Analytics & Export)
*   **商品分析 (Analytics)**:
    *   階層フィルターを適用して配下率、売上、客数などのメトリクスを集計表示。
...
### 2.2 棚割指示書 (Planogram Instruction Sheet)
店舗で棚替え作業を行うためのPDF帳票機能。

*   **出力対象**: 特定の個店棚割
*   **フォーマット**: UI表示からのブラウザ印刷によるPDF
*   **内容**:
    *   **ヘッダー**: 指示書タイトル、店舗情報、適用日。
    *   **フロア全体図**: フロアでの什器の配置座標・回転を加味したレイアウト図。
    *   **詳細図**: 什器タイプごとの棚段ベース商品配置図（2Dグリッド、商品名とフェイス数表示）。
    *   **ブロック詳細**: ブロックの構成表とフェイスのリスト表示。
    *   **注釈情報**: ユーザーが追加した変更点などのカスタム注記。

### 2.3 商品採用リスト (Product Placement List)
本部分析用のデータファイル。
